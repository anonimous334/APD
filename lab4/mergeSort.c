...
void* threadFunc(void *var) {
	...
	for (width = 1; width < N; width *= 2) {
		int lowBound = (int)ceil(1.0f * N/P) * thread_id / (2 * width) * (2 * width);
		int upBound = (int)fmin(ceil(1.0f * N/P) * (thread_id + 1), N) / (2 * width) * (2 * width);
		for (int i = lowBound; i < upBound; i += 2 * width) {
